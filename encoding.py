...
# 导入 RARE 模型
from rare import build_model  # 假设 RAREModel 是您的 RARE 模型类
...
# 初始化 RoBERTa 分词器和模型
model_checkpoint = '/root/autodl-tmp/whoiswho/roberta-base'
tokenizer = AutoTokenizer.from_pretrained(model_checkpoint)
model = AutoModel.from_pretrained(model_checkpoint).to(device)

# 初始化 RARE 模型
rare_model = build_model(args)
...
for ii in tqdm(range(0, len(papers), batch_size), total=len(papers) // batch_size):
    ...
    texts = [(paper[1]["title"] + " " + paper[1]["abstract"] + " " + " ".join(paper[1]["keywords"])) for paper in batch_papers]
